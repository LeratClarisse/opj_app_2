...
class DetailsDPS extends StatelessWidget {
  ...
  final prefixStyle = const TextStyle(fontWeight: FontWeight.bold, fontSize: 16);
  final titleStyle = const TextStyle(fontWeight: FontWeight.bold, fontSize: 18);
    ...
    return Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(dpsIntention),
            const SizedBox(height: 20),
            Text('Tentative', style: prefixStyle),
            const SizedBox(height: 10),
            Text(dpsPunissable),
            const SizedBox(height: 30),
            Text('Elément matériel', style: prefixStyle),
            const SizedBox(height: 10),
            Text(dpsElemMat.replaceAll(r'\n', '\n')),
            if (dpsDesc.isNotEmpty) const SizedBox(height: 30),
            if (dpsDesc.isNotEmpty) Text('Description', style: prefixStyle),
